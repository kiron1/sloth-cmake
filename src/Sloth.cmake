...
function(sloth_project_impl _argsout _name)
  ...
  set(_requires)
  set(_args)
  set(_keys "(VERSION|LANGUAGES|COMPATIBILITY|NAMESPACE|REQUIRES)")
  foreach(_word ${ARGN})
    if(_state STREQUAL "NEUTRAL" AND _word MATCHES "${_keys}")
      set(_state "${CMAKE_MATCH_1}")
      string(TOLOWER "${_state}" _key)
    elseif(_state STREQUAL "NEUTRAL")
      list(APPEND _args ${_word})
    elseif(_state MATCHES "(VERSION|COMPATIBILITY|NAMESPACE)")
      set(_state "NEUTRAL")
      set(_${_key} ${_word})
    elseif(_state MATCHES "(LANGUAGES|REQUIRES)")
      if(_word MATCHES ${_keys})
        set(_state "${CMAKE_MATCH_1}")
      else()
        list(APPEND _${_key} ${_word})
      endif()
    endif()
  endforeach()

  set(_args_list)
  if(_languages OR _args)
    list(APPEND _args_list LANGUAGES ${_languages} ${_args})
  endif()
  if(_version)
    list(APPEND _args_list VERSION ${_version})
  endif()
  set(${_argsout} ${_args_list} PARENT_SCOPE)
  ...
  if(_requires)
    set_property(GLOBAL PROPERTY ${_name}_REQUIRES ${_requires})
  endif()
  ...
  if(_version)
    ...
    set_property(GLOBAL PROPERTY ${_name}_COMPATIBILITY "${_compatibility}")
